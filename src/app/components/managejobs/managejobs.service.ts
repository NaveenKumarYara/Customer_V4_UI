...
export class ManageJobService {
  ...
  getFilteredJobDetails(customerId: number, userId: number, sortBy: number, searchString: string, count: number,minSal: number, maxSal: number,minExp: number,maxExp: number,jobStatus: number,locations:string,skills:string,clients:string,departments:string,titles:string,domain:string,immigration:string,lastWeek:string,lastTwoWeek:string,last30days:string,last90days:string,lastyear:string,today:string,category:string,empType:string): Observable<JobDetails> {
    const url = this.settingsService.settings.listofFilteredJobs +'customerId=' + customerId + '&userId=' + userId + '&sortBy=' + sortBy + '&searchString=' + searchString + '&status=0&pageNumber=1' + '&numberOfRows=' + count+'&minSal=' +minSal+'&maxSal='+maxSal+'&minExp='+minExp+'&maxExp='+maxExp+'&jobStatus='+jobStatus+'&locations='+locations+'&skills='+skills+'&clients='+clients+'&departments='+ departments+'&titles='+titles+'&domain='+domain+'&immigration='+immigration+'&lastWeek='+lastWeek+'&lastTwoWeek='+lastTwoWeek+'&last30days='+last30days+'&last90days='+last90days+'&lastyear='+lastyear+'&today='+today+'&category='+category+'&empType='+empType;
    return this.http.get<JobDetails>(url)
      .debounceTime(1000)
      .catch(
        this.handleError
