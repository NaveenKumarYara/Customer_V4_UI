...
export class ManageJobService {
  ...
  getFilteredJobDetails(customerId: number, userId: number, sortBy: number, searchString: string, count: number,minExp: number,maxExp: number,minSal: number, maxSal: number,jobStatus: number,locations:string,skills:string,clients:string,departments:string,titles:string,domain:string,immigration:string,lastWeek:string,lastTwoWeek:string,last30days:string,last90days:string,lastyear:string,today:string,category:string,empType:string,education:string): Observable<JobDetails> {
